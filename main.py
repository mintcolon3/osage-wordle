...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(
        ...
        name="INABAKUMORI | Hadal Abyss Zone"),
...
async def on_message(message):
    ...
    if message.content.startswith(
            '!^ ') and message.author.id == 1170381506460536905:
        ...
        if " -ds " in message.content:
            ...
            reply_to = await message.channel.fetch_message(
                message.reference.message_id)
            if message.content.endswith(' -d'):
                await reply_to.author.send(
                    message.content.replace('!^ ', '').replace(' -d', ''))
            else:
                await reply_to.reply(message.content.replace('!^ ', ''))
        else:
    ...
    if str(message.author.id) in streaks.keys():
        if streaks[str(message.author.id)]["playing"] == 0:
            ...
            if content not in words[1] and content not in words[
                    0] and wordleresponse == "That's not a real word.":
                await message.add_reaction("❌")
                return

            game = await message.channel.fetch_message(streaks[str(
                message.author.id)][str(day)][1])
            ...
            for i in range(5):
                letter = content[i]
                if streaks[str(message.author.id)][str(day)][0][
                        string.ascii_lowercase.index(letter)] == 0:
                    streaks[str(message.author.id)][str(day)][0][
                        string.ascii_lowercase.index(letter)] = 3
                if letter == word[i]:
                    value = 1
                    word[i] = "1"
                    streaks[str(message.author.id)][str(day)][0][
                    ...
                    value = 3
                guess += emojis.letters[value][string.ascii_lowercase.index(
                    letter)]
                guessval += str(value)
            word = "".join(word)
            ...
            keyboard = [""] * 3
            for i in range(10):
                keyboard[0] += emojis.letters[int(streaks[str(
                    message.author.id)][str(day)][0][i])][i]
            for i in range(10):
                keyboard[1] += emojis.letters[int(streaks[str(
                    message.author.id)][str(day)][0][i + 10])][i + 10]
            for i in range(6):
                keyboard[2] += emojis.letters[int(streaks[str(
                    message.author.id)][str(day)][0][i + 20])][i + 20]
            keyboard = "\n".join(keyboard)
            gametext = f"# {'OSAGE WORDLE' if message.author.id != 699418679963811870 else 'OSAGE WORLDE'} #{day}\n{keyboard}\n{'-'*15}\n"
            prev_guesses = game.content.split("\n")[5:]
            if emojis.blank not in prev_guesses[0]:
                gametext += ("\n".join(prev_guesses) + "\n")
            else:
                prev_guesses = []
            gametext += guess
            ...
            if guessval == "11111" or streaks[str(
                    message.author.id)]["playing"] == 6:
                game = await message.channel.fetch_message(streaks[str(
                    message.author.id)][str(day)][1])

                saucestring = words[4][words[1][(day - 1) % len(words[1])]]
                if saucestring.startswith("https") or saucestring == "":
                    sauce = saucestring
                else:
                    sauce = f"https://www.youtube.com/watch?v={saucestring}"

                gametext = game.content + f"\n\nThe word was `{words[1][0 if day == 13 else (day-1)%len(words[1])]}`" + f"\n{sauce}"
                await game.edit(content=gametext)

                image = await gen.genimg(
                    streaks[str(message.author.id)][str(day)][2],
                    message.author, day, gen.imagethemes["gradient"],
                    gen.gamethemes["osagle"])
                image.save(f"exports\{message.id}.png")
                await message.channel.send(
                    file=discord.File(f"exports\{message.id}.png"))
        ...
        game = await message.channel.send(gametext)
        streaks[str(message.author.id)] = {
            "options": [0],
            "streak": 0,
            "playing": 1,
            str(day): [[0] * 26, game.id, []]
        }
...
async def getstreak(ctx, user: discord.User = None):
    async with ctx.typing():
        ...
        if str(user.id) not in streaks.keys():
            ...
            return
        await ctx.reply(
            f"{user.name} has an osage wordle streak of {streaks[str(user.id)]['streak']} days."
        )
...
async def leaderboard(ctx,
                      day: typing.Optional[int] = words[2],
    ...
    async with ctx.typing():
        ...
        lb = dict(
            sorted(streaks.items(),
                   key=lambda item: streakvalue(item[1], day)))
        ...
        for i in range(len(users[:3])):
            ...
            if value != "8":
                ...
                game = gen.gentext(streaks[str(user_ids[i])][str(day)][2],
                                   gen.textthemes[theme])
                game = game.split("\n")
                for guess in game:
                    message += f"\n> {guess}"
